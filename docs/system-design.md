...
**Priority** (per-user, for ordering and caps) — relevance-led, boosted by `max_severity` and by **cross-source corroboration**, and aged by **recency decay** over `now − last_event_time` (applied at read time). v1 is retrospective only; the generalization to a signed-gap salience curve that also handles future-valued items is deferred (§8.5). **Corroboration is implemented** (M4, a deterministic term — not ML, not tied to Threads): `source_diversity` feeds *priority* — `priority ×(1 + corroboration_weight · (1 − 1/source_diversity))`, saturating — not only richness/format, because independent sources corroborating one story is *importance*, not a render hint. The remaining priority term, **reactivation/novelty** (a bump when a story lands on a dormant thread or a thread spikes over its baseline rate), is deferred with Threads (§8.6).
